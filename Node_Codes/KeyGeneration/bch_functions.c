...
// #  include <winsock2.h>
// #  define alloca_s(size) _alloca(size)
#  define alloca_s(size) malloc(size)
...
static inline unsigned int fls_s(unsigned int x)
{
		...
		unsigned int len, uint8_t *ecc)
{
	...
	unsigned int i, mlen;
	unsigned long m;
	uint32_t w, *r = alloca_s(sizeof(uint32_t) * (l+1));
			       ...
			       unsigned int *sol, int nsol)
{
	...
	int rem, c, r, p, k, *param = alloca_s(sizeof(int) * m);
...
static int build_deg2_base(struct bch_control *bch)
{
	...
	unsigned int *xi = alloca_s(sizeof(unsigned int) * m);
...
{
//    const int m = 8, prim_poly = 285; // parameters set for 128-bit message 285
	const int m = 13, prim_poly = 8219; 
//    int t = 19;
    ...
    memcpy(&pResult[payloadLen], ecc, m*t/8);
...
//    int t = ECC_BYTES;
    ...
    if (pInput == NULL || pPayload == NULL || pPayloadLen == NULL) {
        return -1; 
    }
    
    *pPayloadLen = inputLen-t;
    
    uint8_t * ecc = (uint8_t*)calloc(m*t/8, sizeof(char));
    unsigned int err_location[ECC_BYTES];
    
    struct bch_control * bch = init_bch(m,t,prim_poly);
    
    memcpy(pPayload, pInput, *pPayloadLen);
    memcpy(ecc, &pInput[inputLen-t], m*t/8);
